...
export default async function fetchRecipeSuggestions(pantryItems) {
  const prompt = `Suggest 4 finished recipes using the following ingredients in less than 150 words and each recipe in with only 30 words and also if items are not available return stock is empty message: ${pantryItems.join(
    ", "
  )}`;
