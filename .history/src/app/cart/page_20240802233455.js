...
export default function Cart() {
  ...
  const [shouldRefreshItems, setShouldRefreshItems] = useState(false); // New state to control refresh
  ...
  useEffect(() => {
    if (shouldRefreshItems) {
      const fetchItems = async () => {
        ...
        setShouldRefreshItems(false); // Reset flag after refreshing
      };
      fetchItems();
    }
  }, [shouldRefreshItems]);
  ...
  const addItem = async (e) => {
    ...
    if (newItem.name !== "" && newItem.quantity !== "") {
      ...
      setShouldRefreshItems(true); // Set flag to refresh items
    }
  };
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <main className="flex min-h-screen flex-col items-center justify-between p-24">
        <div className="z-10 max-w-5xl w-full font-mono text-sm">
          ...
          <Button
            onClick={() => setShouldRefreshItems(true)}
