...
export default async function fetchRecipeSuggestions(pantryItems) {
  const prompt = `Suggest only 4 finished recipes using the following ingredients in less than 150 words and each recipe in with only 30 words: ${pantryItems.join(
    ", "
  )}`;
