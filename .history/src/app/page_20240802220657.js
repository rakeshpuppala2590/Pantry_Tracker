...
export default function Home() {
  ...
  const addItem = async (e) => {
    ...
    if (
      ...
      await addDoc(collection(db, "items"), {
        ...
        quantity: parseFloat(newItem.quantity), // Ensure quantity is a number
        date: newItem.date.toISOString(), // Convert to ISO string
      });
  ...
  const updateItem = async (e) => {
    ...
    if (upItem.name && upItem.price && upItem.quantity && upItem.date) {
      const itemName = upItem.name.trim().toLowerCase(); // Ensure consistent case for comparison
      console.log("Searching for item with name:", itemName); // Log the name being searched
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          console.log("Found item with ID:", itemId); // Log the found item ID
          ...
          console.log("Item not found with name:", upItem.name);
