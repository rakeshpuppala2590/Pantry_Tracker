...
export default function Cart() {
  ...
  // Delete Item from cart DB and update items collection
  const deleteItem = async (id) => {
    try {
      // Get the item details from the cart
      const cartDoc = await doc(db, "cart", id).get();
      if (!cartDoc.exists()) {
      ...
      // Restore the quantity to the items collection
      const itemDoc = doc(db, "items", cartItem.id);
      const itemData = (await itemDoc.get()).data();

      if (itemData) {
        await updateDoc(itemDoc, {
          quantity: itemData.quantity + parseInt(cartItem.quantity),
        });
      }
