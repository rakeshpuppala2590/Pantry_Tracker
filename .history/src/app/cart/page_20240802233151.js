...
export default function Cart() {
    ...
    if (newItem.name !== "" && newItem.quantity !== "") {
      ...
      await Promise.all([
        ...
        addDoc(collection(db, "cart"), {
          ...
          itemId: item.id, // Add itemId to cart data
        }),
  ...
  const deleteItem = async (id) => {
    try {
      ...
      // Ensure itemId is present in cartData
      if (!cartData.itemId) {
        console.error("ItemId is missing in cartData");
        return;
      }

      // Find the item in the availableItems collection
      const itemDocRef = doc(db, "items", cartData.itemId); // Use itemId from cartData
      const itemDoc = await getDoc(itemDocRef);
      const itemData = itemDoc.data();

      if (!itemData) {
        console.error("Item not found in items collection");
        return;
      }
