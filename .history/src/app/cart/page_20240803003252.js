...
export default function Cart() {
  ...
  const deleteItem = (id) => {
    // Create a new Set and add the item ID to it
    setRemovedItems((prevRemovedItems) => {
      const updatedRemovedItems = new Set(prevRemovedItems);
      updatedRemovedItems.add(id);
  ...
  const confirmChanges = async () => {
    ...
    for (const id of removedItems) {
      ...
      if (cartItem) {
        ...
        const q = query(collection(db, "cart"));
        const querySnapshot = await getDocs(q);
        const docs = querySnapshot.docs; // Array of documents

        // Process documents in chunks
        for (let i = 0; i < docs.length; i += BATCH_SIZE) {
          const batch = writeBatch(db); // Create a new batch
          const chunk = docs.slice(i, i + BATCH_SIZE); // Get a chunk of documents
