  ...
  const addItem = async (e) => {
    ...
    if (
      ...
      try {
        ...
        const response = await fetch("/api/route/category", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ image: imageUrl }), // Send image URL or base64 if necessary
        });
        ...
        const category = data.category || ""; // Handle undefined category

        // Add the item with the image URL and category to the database
