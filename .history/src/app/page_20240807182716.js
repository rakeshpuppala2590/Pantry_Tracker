...
export default function Home() {
  ...
  const handleCapture = (imageFil) => {
    setNewItem({ ...newItem, imageFile: imageFil });
    console.log(imageFil);
    setImageFileName(imageFil.name); // Set the image file name
                  ...
                  <div className="fixed top-0 left-0 w-full h-full bg-black flex flex-col items-center justify-center z-50">
                    <div className="relative">
                      <Camera
                        ref={cameraRef}
                        onTakePhoto={(file) => {
                          // Directly handle the File object
                          handleCapture(file);
                        }}
