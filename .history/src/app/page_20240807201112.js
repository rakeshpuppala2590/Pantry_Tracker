  ...
  const handleCapture = async () => {
    try {
      if (camera.current) {
        ...
        console.log("Photo taken:", photoData.substring(0, 50) + "..."); // Log first 50 characters

        const base64Image = photoData.split(",")[1];
        console.log("Base64 image extracted");

        setCapturedImage(photoData);
        console.log("Captured image set");

        console.log("Classifying image...");
        const category = await classifyImage(base64Image);
        console.log("Image classified:", category);

        setNewItem({
          ...newItem,
          category: category,
          imageFile: dataURItoBlob(photoData),
        });
        console.log("New item state updated");
